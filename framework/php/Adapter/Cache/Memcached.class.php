...
class Adapter_Cache_Memcached implements Adapter_Cache_Interface {
	...
	private function __construct() {
		...
		if(!$this->boolConnected) {
			throw new Exception_Cache_NotConnected($objConfig->CACHE_SERVER, $objConfig->CACHE_PORT);
		}
	}
	...
	public function flush() {
		if (!$this->objMemcache->flush()) {
			throw new Exception_Cache_UnableToFlush("Memcache error: " . $this->objMemcache->getResultCode());
		}
		return true;
	}
	...
	public function getTotalSpace() {
		...
		if ($arrStats === false) {
			throw new Exception_Cache_UnableToRetrieveStats($this->memcached->getResultMessage());
		}
